...
const welcomePost = {
  ...
  description: "An example blog post showing how to author content with MDX in this template.",
  slug: "welcome",
  author: "PageZERO",
}
...
test("blog index and post pages", async ({ page }) => {
  ...
  await expect(page).toHaveTitle(`${welcomePost.title} - ${config.core.projectName}`)
  await expect(page.getByRole("heading", { level: 1, name: welcomePost.title })).toBeVisible()
  await expect(page.getByText(welcomePost.author, { exact: true })).toBeVisible()
