...
test("newsletter double opt-in flow (dev email capture)", async ({ page }) => {
  ...
  await expect(page.getByText("Check your email to confirm your subscription")).toBeVisible()

  const emails = (await page.evaluate(() =>
    fetch("/emails/sent").then((res) => res.json()),
  )) as DevelopmentEmailPayload[]

  const newsletterEmails = emails.filter((e) => e.subject.toLowerCase().includes("newsletter"))
  ...
  const toField = Array.isArray(lastEmail.to) ? lastEmail.to.join(", ") : lastEmail.to
  expect(toField).toContain(testEmail)

  const urlMatches = [...lastEmail.body.matchAll(/https?:\/\/[^\s<>"')]+/g)].map((m) => m[0])
  const confirmUrlRaw = urlMatches.find((u) => u.includes("/newsletter/confirm?"))
  ...
  await expect(page.getByRole("heading", { name: "Newsletter", level: 2 })).toBeVisible()
